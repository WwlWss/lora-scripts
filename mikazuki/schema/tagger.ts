Schema.intersect([
    Schema.object({
        interrogator_model: Schema.union([
            "cl_tagger_1_01",
            "wd-convnext-v3",
            "wd-swinv2-v3",
            "wd-vit-v3",
            ...
            "animetimm-caformer-m36-dbv4",
            "animetimm-caformer-b36-dbv4",
            "animetimm-eva02-large-dbv4",
            "danbooru-tag-query-b16",
            "danbooru-tag-query-l16",
            "pixai-tagger-v0.9"
        ]).default("wd-vit-v3").description("Tagger 模型"),
        path: Schema.string().role('folder').required().description("图片文件夹路径"),
        threshold: Schema.number().role("slider").min(0).max(1).step(0.01).default(0.35).description("通用标签阈值"),
        character_threshold: Schema.number().role("slider").min(0).max(1).step(0.01).default(0.6).description("角色名称识别阈值（支持角色分类的 Tagger 生效）"),
        ...
        replace_underscore: Schema.boolean().default(true).description("使用空格代替下划线（适合 Anima）"),
